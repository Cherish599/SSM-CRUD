...
<body>
	...
	<script type="text/javascript">
		...
		//1、页面加载完成以后，我们去发送一个ajax请求，要到分页数据
		$(function(){
			//去首页			
			to_page(1);	
		});
		...
		$("#emp_add_modal_btn").click(function(){
			//清除表单数据（表单重置(表单完整重置，包括表单的数据，表单的样式)）
		...
		function validate_add_form(){
			...
			if(!regEmail.test(email)){
				//alert("邮箱格式不正确！");
				//应该清空这个元素之前的样式。
				show_validate_msg("#email_add_input","error","邮箱格式不正确！1");
				return false;
			}else{
		...
		$("#emp_save_btn").click(function(){
			...
			//2、判断之前的用户名校验是否成功了。如果成功了，才往下继续走
		...
		//1、我们是按钮创建之前就绑定了click，所以绑定不上click事件
		...
		$("#emp_update_btn").click(function(){
			...
			var regEmail = /^([a-z0-9_\.-]+)@([\da-z\.-]+)\.([a-z\.]{2,6})$/;
			if(!regEmail.test(email)){
				show_validate_msg("#email_update_input","error","邮箱格式不正确！1");
				return false;
			}else{
